...
function loadDestinations(destinations) {
  ...
  destinations.forEach(function(destinationConfig){
    if (destinationConfig.proxyType === 'onPremise' && destinationConfig.forwardAuthToken){
      throw new Error('ForwardAuthToken parameter cannot be used in destinations with proxyType onPremise');
    }
  });
