...
var whitelistUtil = require('../utils/whitelist-utils');
...
module.exports = function processCors(req, res, next) {
  ...
  var corsDefinition = getCorsDefinition (req);
  if (!corsDefinition) {
    return next();
  }

  var corsConfigWithDefaults = fillDefaultsCorsConfig(corsDefinition);
...
function getCorsDefinition (req) {
  var cors = req.routerConfig.cors;
  return _.find(cors, function (corsObj) {
    return corsObj.uriPattern.test(req.url);
  });
}
...
function isLocalOrigin (req, origin) {
  ...
  var protocol = appRouterUrl.protocol.slice(0, -1);
  if (!protocol) {
    return false;
  }
  target += protocol + '://';

  var host = appRouterUrl.hostname;
  if (!host) {
    return false;
  }
  target += host;

  var port = appRouterUrl.port;
...
function processActualRequest (req, res, originURL, corsDefinition, next) {
  ...
  var anyOriginAllowed = isAnyOriginAllowed (corsDefinition);
  ...
  if (corsDefinition.exposeHeaders && corsDefinition.exposeHeaders.length > 0) {
    var exposedHeaders = corsDefinition.exposeHeaders.join(', ');
...
function processPreflight(req, res, originURL, corsDefinition, next) {
  ...
  var hasMethodMatch  = _.includes(corsDefinition.allowedMethods, requestMethod);
  if (!hasMethodMatch) {
  ...
  if (header) {
    var requestHeaders = header.replace(' ', '').split(',');
    var headersMatching = _.intersection(_.map(requestHeaders, _.toLowerCase), _.map(corsDefinition.allowedHeaders, _.toLowerCase));
  ...
  var supportCredentials = corsDefinition.allowedCredentials;
  ...
  var maxAge = corsDefinition.maxAge;
  if (maxAge > 0) {
