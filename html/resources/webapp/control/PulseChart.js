	...
	function(Control, d3, DateFormat, JSONModel, PulseChartItem, Formatter, VBox, FlexAlignItems, FlexJustifyContent, FlexBox) {
		...
		return Control.extend("sap.fiori.cri.control.PulseChart", {
			...
			metadata: {
				...
				aggregations: {
					reactions: {
						type: "sap.fiori.cri.model.PulseChartItem",
						multiple: true,
			...
			refresh: function() {
				...
				this.svg.append("linearGradient")
					...
					.data([{
						...
						offset: thresholdLowValue + "%",
						color: colorScale(thresholdLowValue)
					}, {
						offset: thresholdHighValue + "%",
						color: colorScale(thresholdHighValue)
					}])
					.enter().append("stop")
				...
				dataPoints
					...
					.attr("stroke", function(d) {
						...
						return d.items.filter(function(el) {
							if (el.icon) {
								return (el.icon);
							}
						}).length === 1 ? "white" : colorScale(d.value);
					})
					.attr("fill", "white")
					.attr("r", 6);
				...
				dataPoints
					...
					.attr("stroke", function(d) {
						...
						}).length > 0 ? "white" : colorScale(d.value);
					})
				...
				pointLabels
					...
					.text(function(d) {
						...
						return sap.ui.core.IconPool.getIconInfo(d.items.filter(function(el) {
							if (el.icon) {
								return (el.icon);
							}
						}).sort(function(a, b) {
			...
			//extract distinct icons for legend from array of icons
				...
				if (bShowSimplePulse) {
					...
					//polyfill or remove() for Nodelist
					(function() {
					...
					oPulseChildren.forEach(function(oChild) {
						if (oChild.className.baseVal === "x axis") {
							while (oChild.childNodes.length !== 3) {
								oChild.childNodes[1].remove();
							}
						} else if (oChild.className.baseVal === "y axis") {
							while (oChild.childNodes.length !== 4) {
								oChild.childNodes[1].remove();
							}
						} else if (oChild.className.baseVal === "zoom-brush" || oChild.className.baseVal === "zoom-reset") {
							oChild.remove();
						}
					});
