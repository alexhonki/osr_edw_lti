		...
		return BaseController.extend("sap.fiori.cri.controller.EventOverview", {
			...
			getVisualFilterData: function(oData) {
				...
				oData.forEach(function(filterRange) {
					if (filterRange.BUCKET_ID == "DAYS_TO_REACT" || filterRange.BUCKET_ID == "INFLUENCE" || filterRange.BUCKET_ID == "IMPACT") {
			...
			onSelectVisualFilter: function(event) {
				...
				if (event) {
					...
					if (selectedFilters) {
						...
						selectedFilters = [selectedVisualFilter];
					}
				}
				selectedFilters.forEach(function(filter) {
			...
			initBubbleChart: function() {
				...
				var oTemplate = new VisualFilterPlotItem({
					...
					xValue: {
						path: this.oBubbleChartFields.x,
						formatter: this.validateNumber
					},
					yValue: {
						path: this.oBubbleChartFields.y,
						formatter: this.validateNumber
					}
				});
			...
			onTopEventSelection: function(event) {
				var selectedEvent = event.getParameters().listItem.getBindingContext().getModel().getProperty(event.getParameters().listItem.getBindingContext()
					.getPath());
				var currentEvent = FilterService.getFilterValue("SelectedEvent", this.EventSelectionStorageKey);
				if (!selectedEvent || currentEvent && currentEvent.EVENT_GROUP == selectedEvent.EVENT_GROUP && currentEvent.EVENT_NAME ==
			...
			updateTopEvents: function() {
				var oTopEvents = [{
						id: "topRiskEventList",
						templateId: "topRiskEventItemTemplate",
						sortPath: "INFLUENCE",
						sortDesc: true
					}, {
						id: "topImpactEventList",
						templateId: "topImpactEventItemTemplate",
						sortPath: "IMPACT",
						sortDesc: true
					}
					//,
			...
			onSelectVisualFilterBubbles: function(event) {
				...
				eventList.forEach(function(event) {
					if (event.IMPACT <= maxImpact && event.IMPACT >= minImpact && event.INFLUENCE <= maxInfluence && event.INFLUENCE >= minInfluence) {
			...
			selectEvent: function(event) {
				...
				var custListPath = "/CustomerByEventParameters(IP_EVENT_ID='" + encodeURIComponent(eventId) + "'," + sDateRangeFilters +
					",IP_FILTER='" + sFilterString + "')/Results";
			...
			updateEventCategories: function() {
				...
				if (aEvents) {
					aEvents.forEach(function(oEvent) {
						...
						if (aEventCategories.length == 0) {
							aEventCategories.push({
								"EVENT_GROUP": sEventGroup
							});
						} else {
				...
				aEventCategories = [{
					"EVENT_GROUP": this.AllCategoriesKey
				}].concat(aEventCategories);
			...
			bucketColorFormatter: function(iBucketIndex) {
				switch (iBucketIndex) {
					case 3:
						return this.oBucketColor.high;
					case 2:
						return this.oBucketColor.medium;
					case 1:
					default:
			...
			exportEventList: function() {
				...
				aEventContexts = this.getView().byId("idEventListTable").getBinding("rows").getContexts();
				aEventContexts.forEach(function(item) {
					aEventList.push(item.getObject());
				});

				//var aEventList = oModel.getProperty("/SelectedEvents") ? oModel.getProperty("/SelectedEvents") : oModel.getProperty("/DisplayedEvents");
				var arrayOfData = [
					["Event Group", "Event Name", "Impact", "Influence", "Total Customer"]
				];
				if (aEventList) {
			...
			exportCustomerList: function() {
				...
				if (table.getSelectedIndices().length > table.getBinding("rows").getContexts().length) {
					...
					var sDateRangeFilters = this.getDateRangeFilterString();
					var sFilterString = FilterService.getEncodedFilterString() || "";
					sCustomerReadPath = "/CustomerByEventParameters(IP_EVENT_ID='" + encodeURIComponent(eventId) + "'," + sDateRangeFilters +
						",IP_FILTER='" + sFilterString + "')/Results";
			...
			prepareCustomerListForExport: function(aCustomers, oTable) {
				...
				selectedCustomers.forEach(function(oContext) {
					...
					customersToExport.push({
						...
						INIT_DATE: data.INIT_DATE ? data.INIT_DATE.toISOString().split('T')[0] : null
					});
				});

				this.getOwnerComponent().exportDialog.open(this.getView(), customersToExport, true);
			}
		});
	});